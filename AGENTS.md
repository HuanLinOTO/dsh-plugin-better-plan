...
双半（host + client）DSH 插件，取代内置 plan mode 的计划交付与审批面：模型先把完整计划写成 markdown 文件，再调用同名 `exit_plan_mode({ path })`；插件读文件 → 经自有 WS 推送到 better-sidebar 的「Plan」tab。推送**送达已连接视图**时工具调用停靠在审批门上——对话直接暂停，聊天不弹审批卡，用户在 Plan 面板动作栏点 Approve / Keep planning（可附反馈），决定经 `POST /better-plan/api/review` 结算。推送未送达时回退原版聊天审批卡（detail=完整计划全文）。审批语义与模式切换语义与原版一致。
...
- **审批门（侧边栏审批面）**：推送 `delivered: true` 时 execute **不调 `userQuestions.ask`**（无聊天弹窗），改停靠 `review-gate` 的 `begin()`（阻塞 agent loop，即「对话直接停止」）。结算三路：`decide()`（由 `POST /better-plan/api/review` 触发）、exec.signal abort（reject signal.reason）、`dispose()`（插件重载）。错误文案必须与原版弹窗路径**逐字一致**（keep 反馈 / 重载提示）——模型看到的纠错指引与用户从哪个面决定无关，改文案两处同步。`review` 帧 attach 回放（含 pending）保证刷新恢复；HTTP 路由带陈旧 `id` 守卫（409）防多窗口旧窗口误结算新计划。`delivered: false` 永远走原版 ask 弹窗——不要「统一」成 gate，无侧边栏环境会失去审批能力。
...
| `src/review-gate.ts` | 审批门：per-session 停靠 + `begin`/`decide`/`peek`/attach 回放/abort/dispose 结算 |
| `src/review-route.ts` | `POST /better-plan/api/review`：信任栅栏 + 体校验（4KB 上限）+ 陈旧 id 守卫 |
...
- 审批门与 `pendingExits` 记账都在 host 内存（不落会话日志）：`dsh web` 重启会把停靠的审批以「插件已重载」错误结算（模型需重新提交）；`delivered` 判定取 enqueue 时刻的视图连接数，浏览器关掉再开要靠下一次交付。
