...
function pluginReloadedWhileReviewingError() {
	return /* @__PURE__ */ new Error("the better-plan plugin was reloaded while the plan was under review; write the plan and present it again");
}
/**
...
var PlanReviewGate = class {
	...
	*/
	begin(sessionId, review, signal) {
		...
		if (existing !== void 0) {
			...
			existing.reject(/* @__PURE__ */ new Error("a newer plan delivery superseded the pending review"));
		}
		return new Promise((resolve, reject) => {
			...
			const onAbort = () => {
				...
				reject(signal?.reason instanceof Error ? signal.reason : /* @__PURE__ */ new Error("the plan review was aborted"));
			};
	...
	*/
	decide(sessionId, decision, feedback) {
		...
		waiter.reject(/* @__PURE__ */ new Error(trimmed === "" ? "The user chose to keep planning; revise the plan file and present it again." : `The user chose to keep planning; their feedback: ${trimmed}`));
		return this.settle(sessionId, {
			...waiter.review,
			status: "kept"
		});
	...
	*/
	...
	dispose() {
		for (const [sessionId, waiter] of this.pending) {
			...
			waiter.reject(pluginReloadedWhileReviewingError());
		}
		this.subscribers.clear();
	}
	/**
...
function rewritePlanPolicySection(text, planDir) {
	let result = text;
	const writeException = ` The one required exception is the delivery file: before calling exit_plan_mode, write the complete plan as markdown to a single file (for example ${`\`${planDir}/YYYY-MM-DD-<topic>.md\``}) with the write tool; this delivery write is allowed in plan mode.`;
	...
	const deliveryReplacement = "When ready, call exit_plan_mode with the path of the plan file you wrote: the tool takes only that path, and the complete plan markdown must already be in the file, starting with a # title. Never paste the plan text into the tool call.";
...
function registerPlanPolicyOverride(ctx, planDir) {
	return ctx.on("system-prompt/assemble", async (assembly, _context, next) => {
		...
		for (const section of result.sections) if (section.text.includes("When ready, call exit_plan_mode with the complete plan markdown, starting with a # title.")) section.text = rewritePlanPolicySection(section.text, planDir);
		return result;
	});
}
//#endregion
...
function defineExitPlanTool(deps) {
	...
	return defineTool({
		...
		execute: async (args, exec) => {
			...
			if (delivered) {
				await deps.reviewGate.begin(sessionId, {
					id,
					path: absolute,
					title
				}, exec.signal);
