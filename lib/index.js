...
function exitPlanDescription(config) {
	return `Use only in plan mode. Before calling, write the COMPLETE plan as markdown to a file with the write tool (e.g. \`${config.planDir}/YYYY-MM-DD-<topic>.md\`), then pass its path here. The plan opens in the sidebar plan panel and the call returns immediately — end your turn right after it and wait; the user reviews and decides there. Their decision arrives as the next message: approval switches plan mode off for you to carry out the plan; keep-planning feedback asks you to revise the file and present it again.`;
}
/**
